...
async def test_multicast_groups_remove_device(core: Core, client_session, dev_eui):
    ...
    client_session.post.return_value.__aenter__.return_value.json.return_value = {"is_deleted": 1}

    is_created = await core.multicast_groups.remove_device_from_multicast_group(multicast_group_id=1, dev_eui=dev_eui)
    client_session.post.assert_called_with(
        core._Core__api_endpoint_schema.multicast / "multicast-groups" / "remove-device",
        json={"multicast_group_id": 1, "dev_eui": f"{dev_eui:016x}"},
    )
    assert is_created == 1
...
async def test_multicast_groups_remove_device_parameter_error(core: Core, dev_eui):
    with pytest.raises(exceptions.ParameterError):
        await core.multicast_groups.remove_device_from_multicast_group(multicast_group_id=1, dev_eui=dev_eui)


@pytest.mark.asyncio
async def test_multicast_groups_remove_device_parameter_error_id(core: Core):
    with pytest.raises(exceptions.ParameterError):
        await core.multicast_groups.remove_device_from_multicast_group(
            multicast_group_id="TEST", dev_eui=0xFFFFFFFFFFFFFFFF
        )
...
@pytest.mark.parametrize("dev_eui", [0xFFFFFFFFFFFFFFFF])
async def test_multicast_groups_remove_device_api_error(core: Core, client_session, dev_eui, api_error, exception):
