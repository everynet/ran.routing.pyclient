...
async def test_multicast_groups_add_device(core: Core, client_session, dev_eui):
    ...
    client_session.post.return_value.__aenter__.return_value.json.return_value = {"is_created": 1}

    is_created = await core.multicast_groups.add_device_to_multicast_group(multicast_group_id=1, dev_eui=dev_eui)
...
async def test_multicast_groups_add_device_parameter_error(core: Core, dev_eui):
    with pytest.raises(exceptions.ParameterError):
        await core.multicast_groups.add_device_to_multicast_group(multicast_group_id=1, dev_eui=dev_eui)


@pytest.mark.asyncio
async def test_multicast_groups_add_device_parameter_error_id(core: Core):
    with pytest.raises(exceptions.ParameterError):
        await core.multicast_groups.add_device_to_multicast_group(multicast_group_id="TEST", dev_eui=0xFFFFFFFFFFFFFFFF)
